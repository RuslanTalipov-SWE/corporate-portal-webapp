...
async function deleteCommentAndReplies(commentId: string, prisma: any) {
...
async function deleteCommentsForPost(postId: string, prisma: any) {
...
export async function DELETE(
  ...
  try {
    ...
    await db.$transaction(async (prisma) => {
      // Delete all subscriptions related to the community
      ...
      // Then, delete the posts in the community
    ...
    return new Response(
      JSON.stringify({
        message: "Could not delete community, please try again later.",
        error: errorMessage,
      }),
      {
        headers: { "Content-Type": "application/json" },
